...
public class Evaluator {
    ...
    public int evaluateExpression(String expression ) throws InvalidTokenException {
            ...
            if ( !( expressionToken = this.expressionTokenizer.nextToken() ).equals( " " )) {
                ...
                if (Operand.check(expressionToken)) {
                    ...
                    if (!Operator.check(expressionToken)) {
                        ...
                        operatorStack.push(new LeftParan());

                    } else if (")".equals(expressionToken)) {
                        while (operatorStack.peek().priority() != 0) {
                            Operator operatorFromStack = operatorStack.pop();
                            Operand operatorTwo = operandStack.pop();
                            Operand operatorOne = operandStack.pop();
                            operandStack.push(operatorFromStack.execute(operatorOne, operatorTwo));
                        }
                        operatorStack.pop();
                    } else {
        ...
        while (!operatorStack.empty()){
            Operator operatorFormStack = operatorStack.pop();
            Operand operandTwo = operandStack.pop();
            Operand operandOne = operandStack.pop();
            operandStack.push(operatorFormStack.execute(operandOne,operandTwo));
